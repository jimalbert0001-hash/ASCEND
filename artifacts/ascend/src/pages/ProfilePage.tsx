    ...
    <div className="p-4 md:p-8 space-y-8 max-w-3xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start text-center md:text-left">
          ...
          <div className="flex-1 space-y-4">
            ...
            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
              {profile.activeDomains.map(domain => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-6">
          ...
          <p className="text-3xl font-bold"><EditableField value={profile.studyHours} onSave={(v) => updateProfile({ studyHours: Number(v) })} min={0} className="text-3xl font-bold" /></p>
        </Card>
        <Card className="p-6">
          <p className="text-sm font-medium text-muted-foreground mb-1">Chess Rating</p>
          <p className="text-3xl font-bold tabular-nums"><EditableField value={profile.chessRating} onSave={(v) => updateProfile({ chessRating: Number(v) })} min={0} className="text-3xl font-bold tabular-nums" /></p>
        </Card>
        <Card className="p-6">
          <p className="text-sm font-medium text-muted-foreground mb-1">Habit Streak</p>
          <p className="text-3xl font-bold tabular-nums"><EditableField value={profile.habitStreak} onSave={(v) => updateProfile({ habitStreak: Number(v) })} min={0} className="text-3xl font-bold tabular-nums" /></p>
        </Card>
      </div>

      <div className="text-center text-sm text-muted-foreground mt-12">
        Operator since {profile.joinedDate}
      </div>
    </div>
  );
}
